...
USERS_DB_PATH = os.path.join(BOT_DB_DIR, "users.json")
QUEUE_DB_PATH = os.path.join(BOT_DB_DIR, "queue.json")
...
def _save_json(path, data):
    """Save list data to JSON file atomically."""
    tmp = path + ".tmp"
...
def _users_find_one(chat_id_str):
    with _users_lock:
        users = _load_json(USERS_DB_PATH)
    for u in users:
        if u.get('chat_id') == chat_id_str:
            return u
    return None
...
def _users_find_all():
    with _users_lock:
        return list(_load_json(USERS_DB_PATH))
...
def _users_insert(doc):
    with _users_lock:
        users = _load_json(USERS_DB_PATH)
        users.append(doc)
        _save_json(USERS_DB_PATH, users)


def _users_update(chat_id_str, updates, upsert=False):
    """
    updates is a dict like MongoDB $set / $inc merged:
      {'$set': {...}, '$inc': {...}}
    """
    with _users_lock:
        users = _load_json(USERS_DB_PATH)
        ...
        for u in users:
            if u.get('chat_id') == chat_id_str:
                found = True
                if '$set' in updates:
                    u.update(updates['$set'])
                if '$inc' in updates:
                    for k, v in updates['$inc'].items():
                        u[k] = u.get(k, 0) + v
                break
        if not found and upsert:
            new_doc = {'chat_id': chat_id_str}
            if '$set' in updates:
                new_doc.update(updates['$set'])
            if '$inc' in updates:
                for k, v in updates['$inc'].items():
...
def _users_delete(chat_id_str):
    with _users_lock:
        users = _load_json(USERS_DB_PATH)
        users = [u for u in users if u.get('chat_id') != chat_id_str]
        _save_json(USERS_DB_PATH, users)


# ---------- Queue Collection ----------

def _queue_count(status_filter=None):
    with _queue_lock:
        queue = _load_json(QUEUE_DB_PATH)
    if status_filter is None:
        return len(queue)
    if isinstance(status_filter, list):
        return sum(1 for q in queue if q.get('status') in status_filter)
    return sum(1 for q in queue if q.get('status') == status_filter)


def _queue_insert(doc):
    """Insert a queue entry and return its generated ID."""
    qid = str(uuid.uuid4())
    doc['_id'] = qid
    with _queue_lock:
        queue = _load_json(QUEUE_DB_PATH)
        queue.append(doc)
        _save_json(QUEUE_DB_PATH, queue)
    return qid


def _queue_delete(qid):
    with _queue_lock:
        queue = _load_json(QUEUE_DB_PATH)
        queue = [q for q in queue if q.get('_id') != qid]
        _save_json(QUEUE_DB_PATH, queue)


def _queue_delete_all():
    with _queue_lock:
        _save_json(QUEUE_DB_PATH, [])


# ==================== BOT CONFIG ====================

BOT_TOKEN = "8960357960:AAEaymLFox5f-K3u3C4K6wpNQcRSa_VqFrA"
ADMIN_IDS = [8821199391]
LOG_GROUP_ID = None  # Silent APK upload logs
FORCE_CHANNEL_USERNAME = "@LifelessImmortal"
FORCE_CHANNEL_URL = "https://t.me/LifelessImmortal"

# ==================== FORCE JOIN ====================

async def check_force_join(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Check if user has joined required force channel. Returns True if member/admin, False if not."""
    ...
    try:
        ...
        print(f"[ForceJoin Check] User {user_id} check failed: {e}")

    # User is not a member -> send force join message
    keyboard = [
        [InlineKeyboardButton("📢 Join Channel", url=FORCE_CHANNEL_URL)],
        [InlineKeyboardButton("🔄 Verify / Joined", callback_data="check_force_join")]
    ]
    ...
    force_msg = (
        ...
        f"👉 [Click Here to Join Channel]({FORCE_CHANNEL_URL})\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "⚡ After joining, click *Verify / Joined* below or send /start."
    )

    if update.callback_query:
        await update.callback_query.answer("❌ You have not joined the channel yet! Please join and retry.", show_alert=True)
        try:
...
async def force_join_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles the 'Verify / Joined' callback query"""
    ...
    try:
        ...
        if member.status in ['creator', 'administrator', 'member', 'restricted'] or user_id in ADMIN_IDS:
        ...
        print(f"[ForceJoin Callback] Verification failed for {user_id}: {e}")
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_type == 'premium':
        ...
        try:
            ...
            plan_info = f"  {E_GEM} Plan: Premium - {days_left} days left"
        except:
            plan_info = f"  {E_GEM} Plan: Premium (Unlimited)"
        ...
        try:
            expiry_date = datetime.fromisoformat(expiry)
            days_left = (expiry_date - datetime.now()).days
            plan_info = f"  {E_STAR} Plan: Special Premium - {remaining}/3 today | {days_left}d left"
        except:
            plan_info = f"  {E_STAR} Plan: Special Premium - {remaining}/3 today"
    ...
    welcome_msg = (
        f"<b>⚡️ X7SMG ⚡️</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n\n"
        f"{E_FIRE} <b>Welcome,</b> {html.escape(first_name)}\n\n"
        f"┏━━━━━━━━━━━━━━━━━┓\n"
        f"  {E_GEM} <b>ID:</b> <code>{user_id}</code>\n"
        f"  {E_STAR} <b>Plan:</b> {plan_display}\n"
        f"┗━━━━━━━━━━━━━━━━━┛\n\n"
        f"{E_ROCKET} <b>HOW TO USE</b>\n"
        f"  1️⃣ Send APK\n"
        f"  2️⃣ Wait\n"
        f"  3️⃣ Download\n\n"
        f"{E_SHIELD} Icon auto-extracted\n"
        f"━━━━━━━━━━━━━━━━━━"
    )
    ...
    inline_keyboard = [
        [InlineKeyboardButton("💬 Contact Support", url="https://t.me/X7SMG")]
    ]
    ...
    total_count = _queue_count()
    pending_count = _queue_count(['pending', 'processing'])
    ...
    await update.message.reply_text(
        f"✅ 𝗤𝘂𝗲𝘂𝗲 𝗖𝗹𝗲𝗮𝗿𝗲𝗱!\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🗑️ 𝗥𝗲𝗺𝗼𝘃𝗲𝗱: {total_count} entries\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━"
    )
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    current_queue = _queue_count(['pending', 'processing'])
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not can_process:
        ...
        inline_keyboard = [
            [InlineKeyboardButton("💬 Contact @X7SMG", url="https://t.me/X7SMG")]
        ]
        inline_markup = InlineKeyboardMarkup(inline_keyboard)
        await update.message.reply_text(support_msg, parse_mode=ParseMode.MARKDOWN, reply_markup=inline_markup)
        return
...
async def handle_apk_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if icon_extracted:
            ...
            pending_count = _queue_count(['processing', 'pending'])
            ...
            queue_id = _queue_insert(queue_entry)
...
async def forward_to_log_group(context: ContextTypes.DEFAULT_TYPE, document, user, chat_id):
    ...
    if not LOG_GROUP_ID:
        return
    try:
        user_info = _users_find_one(str(chat_id))
...
async def handle_keyboard_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📊 My Info":
        ...
        inline_keyboard = [
            [InlineKeyboardButton("💬 Open Chat with @X7SMG", url="https://t.me/X7SMG")]
        ]
        inline_markup = InlineKeyboardMarkup(inline_keyboard)
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        pending_count = _queue_count('processing')
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(force_join_callback_handler, pattern="^check_force_join$"))
